...
# Load Helmer age map
age <- rast("Data/Maps_1951-2000/iitf_jgr113_puertorico_forestage_zone_reprojectedWGS84.tif")
...
r77 <- rast("Data/Maps_1951-2000/puerto77_sub1_100905_landcov_urbveg_final.img")
r91 <- rast("Data/Maps_1951-2000/pr91_100805_final_quarry_recode2landcov_subset.img")
...
f <- crop(mask(c(f51, f77, f91, f00), pr), pr)
...
Pred_stack_raw_rp <- terra::project(Pred_stack_raw, crs(r51), method='near')
Pred_stack_rp <- terra::project(Pred_stack, crs(r51), method='near')
...
writeRaster(Pred_stack_raw_rp, "Data/Derived/raw_stack-noFIA.tif", overwrite=T)
writeRaster(Pred_stack_rp, "Data/Derived/thresholded_stack-noFIA.tif", overwrite=T)
...
terra::writeRaster(f_rs, "Data/Derived/Forest_change_resampled_modal.tif", 
                   overwrite=T)
...
terra::writeRaster(Pred_f51_raw, "Data/Derived/Pred_f51_raw-modal.tif")
terra::writeRaster(Pred_f77_raw, "Data/Derived/Pred_f77_raw-modal.tif")
terra::writeRaster(Pred_f91_raw, "Data/Derived/Pred_f91_raw-modal.tif")
terra::writeRaster(Pred_f00_raw, "Data/Derived/Pred_f00_raw-modal.tif")

terra::writeRaster(Pred_f51, "Data/Derived/Pred_f51-modal.tif")
terra::writeRaster(Pred_f77, "Data/Derived/Pred_f77-modal.tif")
terra::writeRaster(Pred_f91, "Data/Derived/Pred_f91-modal.tif")
terra::writeRaster(Pred_f00, "Data/Derived/Pred_f00-modal.tif")

# Sum all of the pixels that were forest in 1951, 1977, 1991, 2000
fcover_51_raw <- global(Pred_f51_raw, "sum", na.rm=T)
fcover_77_raw <- global(Pred_f77_raw, "sum", na.rm=T)
fcover_91_raw <- global(Pred_f91_raw, "sum", na.rm=T)
...
fcover_77 <- global(Pred_f77, "sum", na.rm=T)
fcover_91 <- global(Pred_f91, "sum", na.rm=T)
...
# Area in the life zones layer (polygon)
lz <- sf::st_read("Data/PR_shapes/Lifezones/lifezones_Project.shp")

# Transform the life zone layer into a raster (first get in same CRS)
lz <- st_transform(lz, crs(raster::raster("Data/Derived/Pred_f00_raw-noFIA.tif")))
lzr <- rasterize(lz['ECOZONE'], raster::raster("Data/Derived/Pred_f00_raw-noFIA.tif"))
...
#2 - Proportion of pixel covered in forest 
# (total percentage of each given lifezone covered in forest)
for(i in 1:length(unique(lzr@data@attributes[[1]]$lz))){
  print(i)
  tmp <- f_rs *(lzr2 == i)
