...
# FIA <- read.csv("Data/Derived/FIA_abundnace.csv") # old abundance data (includes vieques etc.)
FIA <- read.csv("Data/Derived/FIA_abundance.csv")
...
Sp_list$binom <- paste0(toupper(substr(Sp_list$binom, 1, 1)), substr(Sp_list$binom, 2, nchar(Sp_list$binom)))
# Give sp code to FIA data
FIA$code <- Sp_list$SP.CODE[match(FIA$SCIENTIFIC_NAME, Sp_list$binom)]
...
x <- x[, c(1,7,2,3,4,5,6)] #this I need to adjust 
colnames(x)[1]<- "CODE"
...
# Euclidean mean distance between habitat patches
...
# write.csv(x,"Data/Derived/master-20240826.csv")
