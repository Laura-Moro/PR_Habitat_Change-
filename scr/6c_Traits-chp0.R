...
data$enn_51 <- lm$value[lm$year==1951 & lm$metric=='enn_mn']
data$enn_00 <- lm$value[lm$year==2000 & lm$metric=='enn_mn']
...
imp_trt <- as.data.frame(tdat_pi$anc_recon[match(tdata_phy$species, 
                                                 rownames(tdat_pi$anc_recon)),])
